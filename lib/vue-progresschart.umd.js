...
// CONCATENATED MODULE: ./packages/progresschart.vue?vue&type=template&id=65f2d842&
...
/* harmony default export */ var progresschartvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    // 画圆
    circle: function circle(cx, cy, r, process, img) {
      var ctx = this.ctx;
    ...
    renderText: function renderText(text, x, y) {
      ...
      if (len === 3) {
        fontWidth = fontWidth - this.fontSize / 15 * len;
      }
    ...
    line: function line(cx, cy, process) {
      ...
      if (this.progressShow) {
        ...
        this.renderText(intProgress, fontLeft, cy * 2 - this.lineWidth * 1.5 + (this.lineWidth - this.fontSize) / 4);
      }
    ...
    frame: function frame(timestamp) {
      ...
      this.frameVal = this.critical(this.frameVal, 0, 100); // 清空画布
      // 清除canvas内容
  ...
  mounted: function mounted() {
    ...
    if (this.bgImg) {
      ...
      img.onload = function () {
        // 等到图片加载进来之后
        _this.imgCanvas = canvasDom.cloneNode(true);

        _this.imgCanvas.getContext('2d').drawImage(img, 0, 0, _this.canvasOpt.width, _this.canvasOpt.height);
