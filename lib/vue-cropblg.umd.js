...
// CONCATENATED MODULE: ./packages/cropblg.vue?vue&type=template&id=78235a08&
...
/* harmony default export */ var cropblgvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    handleImageMove: function handleImageMove(_ref2) {
      var x = _ref2.x,
          y = _ref2.y;
    ...
    handleCropperMove: function handleCropperMove(_ref4) {
      var x = _ref4.x,
          y = _ref4.y;
      ...
      var oX = s.offsetX;
      var oY = s.offsetY;
      var maxX = width - this.cropper.width;
      var maxY = height - this.cropper.height;
      var currentX = x - oX,
          currentY = y - oY; // 判断边界
    ...
    getPointByCoordinate: function getPointByCoordinate(_ref5) {
      ...
      if (this.points.some(function (point, i) {
        index = i;
    ...
    getImage: function getImage() {
      ...
      return new promise_default.a(function (resolve, reject) {
        ...
        if (!_this.$slots.watermark) {
          resolve(types[type](_this.canvas, mimeType));
          return;
        }

        var _this$position = _slicedToArray(_this.position, 3),
            _this$position$ = _this$position[0],
            left = _this$position$ === void 0 ? '50%' : _this$position$,
            _this$position$2 = _this$position[1],
            top = _this$position$2 === void 0 ? '50%' : _this$position$2,
            _this$position$3 = _this$position[2],
            size = _this$position$3 === void 0 ? 1 : _this$position$3;

        var isImg = _this.$slots.watermark && _this.$slots.watermark[0].tag === 'img';

        if (isImg) {
          var watermarkImg = new Image();
          watermarkImg.src = _this.$slots.watermark[0].data.attrs.src;
          watermarkImg.crossOrigin = 'anonymous';

          watermarkImg.onload = function () {
            // 等到图片加载进来之后
            var width = watermarkImg.width * size * _this.scale,
                height = watermarkImg.height * size * _this.scale;

            _this.cCtx.drawImage(watermarkImg, (w - width) * parse_int_default()(left) / 100, (h - height) * parse_int_default()(top) / 100, width, height);
        ...
        var height = 12 * size,
            text = _this.$slots.watermark[0].text.trim() || '请直接放文字',
            width = _this.cCtx.measureText(text).width,
            x = (w - width * 2) * parse_int_default()(left) / 100,
            y = (h + height / 2) * parse_int_default()(top) / 100,
            colorData = _this.cCtx.getImageData(x, y, 1, 1).data; // 变量申请


        _this.cCtx.font = 12 * size + 'px Georgia';
        _this.cCtx.fillStyle = "rgba(".concat(255 - colorData[0], ", ").concat(255 - colorData[1], ", ").concat(255 - colorData[2], ", 1)"); // console.log(this.cCtx.fillStyle)

        if (_this.cCtx.fillStyle === '#ffffff') {
          _this.cCtx.fillStyle = '#000';
        }

        _this.cCtx.fillText(text, x, y);
