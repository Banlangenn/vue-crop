...
// CONCATENATED MODULE: ./packages/cropblg.vue?vue&type=template&id=78e57265&
...
/* harmony default export */ var cropblgvue_type_script_lang_js_ = ({
  ...
  methods: {
    animation: function animation(img) {
      if (img) {
    ...
    draw: function draw() {
      var _this$options2 = this.options,
          ...
          height = _this$options2.height; // 避免预览到背景

      this.ctx.clearRect(0, 0, width, height); // console.time('fillBackground')  

      this.fillBackground(); // console.timeEnd('fillBackground')
      ...
      this.updatePoint(); // console.timeEnd('updatePoint')
      //  console.time('fillCropper')

      this.fillCropper(); // console.timeEnd('fillCropper')
    ...
    drawTouchBar: function drawTouchBar() {
      var ctx = this.ctx,
          ...
          h2 = touchBar.width * 0.15;
      ctx.lineWidth = 4; // ctx.fillStyle = '#000';
      // ctx.fillRect(touchBar.x, touchBar.y, touchBar.width, touchBar.height)

      ctx.fillStyle = color;
      ctx.fillRect(x - touchBar.width / 6, y - touchBar.height / 6, touchBar.width * 0.41, touchBar.height * 0.41);
    ...
    fillImage: function fillImage() {
      ...
      var ctx = this.ctx;
      var rotateAngle = this.rotateAngle || 0;
    ...
    fillCropper: function fillCropper() {
      var ctx = this.ctx,
    ...
    handleLineMove: function handleLineMove(_ref2) {
      var x = _ref2.x,
          y = _ref2.y;
    ...
    handleImageMove: function handleImageMove(_ref3) {
      var x = _ref3.x,
          y = _ref3.y;
    ...
    getCoordinateByEvent: function getCoordinateByEvent(e) {
      var rect = e.target.getBoundingClientRect(),
        ...
        y: touch.clientY - rect.top
          ...
          height = _this$options4.height; // move 到边
    ...
    getPointByCoordinate: function getPointByCoordinate(_ref4) {
      var _this = this;
      ...
      else if (this.points.some(function (point, i) {
          index = i;
          return _this.checkRegion(x, y, point);
        })) {
          t.type = 'handlePointMove';
          this.index = index;
        } // 四根线移动
        else if (this.lines.some(function (line, i) {
            index = i;
            return _this.checkRegion(x, y, line);
          })) {
    ...
    getImage: function getImage() {
      ...
      var image = this.image,
          ...
          types = {
        Base64: function Base64(canvas, mimeType) {
          return new promise_default.a(function (resolve) {
            resolve(canvas.toDataURL(mimeType));
          });
        },
        Blob: function Blob(canvas, mimeType) {
          return new promise_default.a(function (resolve) {
      ...
      var rotateAngle = this.rotateAngle || 0;
      this.canvasRotate('img', cCtx, image.element, rotateAngle, (image.x - cropper.x) * quality, (image.y - cropper.y) * quality, image.width * quality, image.height * quality); // -------------
      // cCtx.drawImage(
      //     image.element,
      ...
      return new promise_default.a(function (resolve, reject) {
        ...
        var _this2$position = _slicedToArray(_this2.position, 4),
            _this2$position$ = _this2$position[0],
            left = _this2$position$ === void 0 ? '50%' : _this2$position$,
            _this2$position$2 = _this2$position[1],
            top = _this2$position$2 === void 0 ? '50%' : _this2$position$2,
            _this2$position$3 = _this2$position[2],
            size = _this2$position$3 === void 0 ? 1 : _this2$position$3,
            _this2$position$4 = _this2$position[3],
            angle = _this2$position$4 === void 0 ? 0 : _this2$position$4; // angle = this.angle || 0


        if (_this2.imageWatermark) {
          var watermarkImg = new Image();
          watermarkImg.src = _this2.getFileSrc(_this2.imageWatermark);
          watermarkImg.crossOrigin = 'anonymous';

          watermarkImg.onload = function () {
            // 等到图片加载进来之后
            var width = watermarkImg.width * size * quality,
                height = watermarkImg.height * size * quality,
                imgX = (w - width) * parse_int_default()(left) / 100,
                imgY = (h - height) * parse_int_default()(top) / 100;

            _this2.canvasRotate('img', cCtx, watermarkImg, angle, imgX, imgY, width, height);

            resolve(types[type](_this2.canvas, mimeType));
          };

          return;
        }

        if (_this2.textWatermark) {
          var height = _this2.limit(12 * size, 12, 100);

          cCtx.font = height + 'px Georgia';
          var text = _this2.textWatermark,
              width = cCtx.measureText(text).width,
              textX = (w - width * 1.031) * parse_int_default()(left) / 100,
              textY = (h - height * 2.82) * parse_int_default()(top) / 100; // 变量申请

          cCtx.fillStyle = _this2.averageColor; // console.log(this.cCtx.fillStyle)

          if (cCtx.fillStyle === '#ffffff') {
            cCtx.fillStyle = '#000';
          }

          _this2.canvasRotate('text', cCtx, text, angle, textX, textY, width, height);
...
//# sourceMappingURL=vue-cropblg.umd.js.map